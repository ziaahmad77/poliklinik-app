            ...
            <form action="{{ route('obat.update', $obat->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <input type="number" name="minimal_stok" value="{{ old('minimal_stok', $obat->minimal_stok) }}" placeholder="5" min="0" step="1"
                            class="w-full px-4 py-2 border-2 rounded-lg focus:border-primary focus:outline-none
                                      @error('minimal_stok') border-red-500 @enderror" required>
                        @error('minimal_stok')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
